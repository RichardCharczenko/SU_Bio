from flask import Flask, render_template, request, url_for #Common to all flaskr files
...
def lacop_form():
    ...
    if request.method == "POST" and form.validate():
        ...
        form_data = buildMutL()
        if plasmidPresent():
            cell = RunLO(form_data[0], form_data[1], int(request.form.get('ALLO')), int(request.form.get('LI')), int(request.form.get('LO')), int(request.form.get('GLU')))
        else:
            cell = RunLO(form_data[0], [], int(request.form.get('ALLO')), int(request.form.get('LI')), int(request.form.get('LO')), int(request.form.get('GLU')))
