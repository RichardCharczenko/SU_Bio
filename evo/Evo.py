...
from evo.SimClimate import Simulate_Climate  # opens other module, makes new object of Simulate_Climate class
from copy import \
    deepcopy  # create new object and recursively copies values from original, any changes are not reflected in original
...
validAlleles = [2, 3, 4, 5, 8, 10,
                20]  # referenced by alleleDef(), contains number of alleles that CAN be tested #still not sure what used for
...
def firstFill(thisGenePool, carryingCapacity):
    ...
    while len(alleleIncreasingFreq) < missingAlleles:  # while the list of alleles is less than the alleles missing
        ...
        while z < len(thisGenePool):  # indicates when the 'window' has scanned through the thisGenePool list once
            alleleIncreasingFreq.append(
                thisGenePool[z])  # appends whatever allele is at this position in thisGenePool[]
            z = z + 1  # increases z to next position in thisGenePool
            if len(
                    alleleIncreasingFreq) == missingAlleles:  # if the allele's added = the alleles needed, the while loop breaks
                break
            ...
            alleleIncreasingFreq.append(
                ran)  # a random choice of allele is picked (random choice was used bc a lot of alleles at the 0 position of thisGenePool[] were being added
...
def newGeneration(genePool, carryingCapacity, weekStartGen, weekSnowsGone, selection):
    ...
    while genePool:  # make and test animals from the whole genePool
        ...
        if animalAve != 0:  # animal is alive after 17 weeks to breed
            nextGenePool.append(
                allele1)  # if animal survives they are added to the next viable list of alleles that can be placed in new generation
            nextGenePool.append(allele2)
        else:  # animal did not survive after 17 weeks
            aliveanddead[
                allele1] = 0  # if animal dies they are kept in a separate allele list that is not able to be pulled from when creating the new generation
...
def runEvo(climate, phenoList, carryingCapacity, years, diversityQuery, selection, ccount, week, alleleQuery, weekStart,
           outPut, chance1, chance2):
    genePool = diversityChoice(diversityQuery, alleleQuery,
                               weekStart)  # returns genes to be used based on amount of diversity wanted
    alleles = deepcopy(
        genePool)  # makes object called alleles and copies values of gene pool into it, keeps track of alleles started with
    ...
    genePool = firstFill(genePool,
                         carryingCapacity)  # NEED scale up alleles to fill habitat at even frequencies, first time only :)b
    ...
    snowWeekValues = dict.values(
        outPut)  # returns a list of all values in the dictionary, these values represent what week each year their will be snow melt
    weekSnowCount = 0
    for year in range(1,
                      years + 1):  # every week # range is (1-20) #range doesn't include stop number #difference in year vs years
        ...
        for weekStartGen in (1, 18, 34):  # These weeks start a new Generation
            ...
            aliveanddead.update(calcAlleleFreqs(
                genePool))  # adds surviving alleles to this dictionary and calculates the frequency of each
            Ordered = OrderedDict(sorted(aliveanddead.items(), key=lambda t: t[
                0]))  # calls ordereddict to sort keys in aliveandead dictionary on increasing numerical order
            ...
            Xaxis = [(x) + 18 for x in
                     Xaxis]  # gives list of the numbered weeks (at the start of each generation) for entire simulation
            allele1.append(Ordered.values()[
                               0])  # allele1 is a list that will be populated by the first key's frequency (first genotype in the genepool dictionary)
            if len(
                    Ordered.values()) > 1:  # allows the second allele's frequencies (and so on) over the course of the simulation be recorded in another list
...
def generate_scenario(section):
    ...
    chance2 = [11, 12, 13,
               14]  # find where is used #never used, only called upon in functions #connection to simclimate value b?
    outPut = {}
    ...
    return runEvo(climate, phenoList, carryingCapacity, years, diversityQuery, selection, ccount, week, alleleQuery,
                  weekStart, outPut, chance1, chance2)
